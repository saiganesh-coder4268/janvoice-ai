...
export default function IssuesPage() {
  ...
  const getSeverityBadge = (severity: string, status: string) => {
    ...
    switch (severity) {
      ...
      case 'low': return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-muted text-muted-foreground border border-border">Low</span>;
  ...
  return (
    <div className="w-full h-full overflow-y-auto bg-muted p-6 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-foreground tracking-tight">All Issues</h1>
            <p className="text-muted-foreground mt-1">Review and manage all civic complaints in Visakhapatnam.</p>
          </div>
          <div className="relative w-full md:w-72">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
            />
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          {loading ? (
            <div className="p-8 flex flex-col space-y-4">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className="animate-pulse flex gap-4 border-b border-border pb-4">
            ...
            <div className="p-12 text-center flex flex-col items-center">
              ...
              <p className="text-muted-foreground">There are currently no civic complaints reported.</p>
            </div>
          ) : (
                  ...
                  {complaints.map((complaint) => (
                    <tr key={complaint.id} className="hover:bg-muted/50 transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-bold text-foreground text-base mb-1">{complaint.title}</p>
                        <p className="text-muted-foreground line-clamp-1 max-w-md">{complaint.description}</p>
                      ...
                      <td className="px-6 py-4">
                        ...
                        <p className="text-xs text-muted-foreground mt-1 ml-5">Ward {complaint.location.ward}</p>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="text-lg font-bold text-foreground">{complaint.priorityScore}</span>
